...
public class RunBatchRouting {
	static public void main(String[] args) throws ConfigurationException, InterruptedException, IOException {
		...
		ScalableRoutingResultWriter resultWriter = new ScalableRoutingResultWriter(cmd.getOptionStrict("output-path"));

		ScalableBatchRouter router = new ScalableBatchRouter(numberOfThreads, batchSize, trackLinks, injector,
				resultWriter);
		router.process(tasks);

		/*
		 * List<RoutingResult> results = router.process(tasks); new
		 * RoutingResultWriter(results).write(newFile(cmd.getOptionStrict("output-path")
		 * )); if (trackLinks) { Map<Id<Link>, Integer> flow = new HashMap<>();
		 *
		 * for (RoutingResult result : results) { result.linkIds.forEach(id ->
		 * flow.compute(id, (id_, c) -> c == null ? 1 : c + 1)); }
		 *
		 * String flowPath = cmd.getOptionStrict("flow-output-path");
		 *
		 * BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new
		 * FileOutputStream(flowPath)));
		 *
		 * writer.write(String.join(";", new String[] { "link_id", "count" }) + "\n");
		 *
		 * for (Map.Entry<Id<Link>, Integer> entry : flow.entrySet()) {
		 * writer.write(String.join(";", new String[] { // entry.getKey().toString(), //
