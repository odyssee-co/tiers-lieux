...
class ScalableBatchRouter {
	private final Injector injector;
	private final int numberOfThreads;
	private final int batchSize;
	private final boolean saveLinks;
	private final ScalableRoutingResultWriter resultWriter;

	public ScalableBatchRouter(int numberOfThreads, int batchSize, boolean saveLinks, Injector injector,
			ScalableRoutingResultWriter resultWriter) {
	...
	public void process(List<RoutingTask> tasks) throws InterruptedException {
		...
		for (int i = 0; i < numberOfThreads; i++) {
			...
			threads.add(new Thread(new Worker(queue, progress, roadNetwork, router)));
		}
	...
	private class Worker implements Runnable {
		...
		Worker(List<RoutingTask> queue, ParallelProgress progress, Network roadNetwork, TripRouter router) {
			this.queue = queue;
		...
		public void run() {
			...
			do {
				...
				for (RoutingTask task : tasks) {
					...
					List<? extends PlanElement> ptPlan = router.calcRoute("pt", originFacility, destinationFacility, 9.0 * 3600.0,
							null);
					...
					taskResults.add(new RoutingResult(task.personId, task.officeId, carTravelTime, carDistance, ptTravelTime,
							ptDistance, linkIds));
				}

				synchronized (resultWriter) {
					try {
						resultWriter.write(taskResults);
					} catch (IOException e) {
